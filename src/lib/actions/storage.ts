...
const STORAGE_PROVIDER = process.env.NEXT_PUBLIC_STORAGE_PROVIDER === 'do_spaces' ? 'do_spaces' : 'local'
...
async function uploadToStorage(key: string, body: Buffer, contentType: string) {
  if (STORAGE_PROVIDER === 'do_spaces' && s3Client) {
    const command = new PutObjectCommand({ Bucket: BUCKET_NAME, Key: key, Body: body, ACL: 'private', ContentType: contentType })
...
export async function uploadFile(file: File, prefix: string, entityId: string | number): Promise<UploadedUrls> {
  if (!file || file.size === 0) throw new Error('No file provided.')
  if (!ALLOWED_FORMATS.includes(file.type)) throw new Error('Invalid file format.')

  const MAX_FILE_SIZE = 10 * 1024 * 1024
  if (file.size > MAX_FILE_SIZE) throw new Error(`File is too large. Maximum size is ${MAX_FILE_SIZE / 1024 / 1024}MB.`)
  ...
  const processedImages = await Promise.all(
    Object.entries(IMAGE_SIZES).map(async ([size, config]) => {
      const webpBuffer = await sharpInstance.clone().resize({ width: config.width, height: config.height, fit: 'inside', withoutEnlargement: true }).webp({ quality: config.quality }).toBuffer()
  ...
  const uploadPromises = [
    ...
    ...processedImages.map((img) => uploadToStorage(`${prefix}/${img.filename}`, img.buffer, 'image/webp')),
  ]
  ...
  if (STORAGE_PROVIDER === 'local') {
    for (const key in uploadedUrls) {
      ...
      if (uploadedUrls[urlKey]) uploadedUrls[urlKey] = `uploads/${uploadedUrls[urlKey]}`
    }
  }
...
async function deleteFromStorage(storagePath: string): Promise<void> {
  ...
  if (STORAGE_PROVIDER === 'do_spaces' && s3Client) {
    ...
    try {
      ...
      if (error.code !== 'ENOENT') console.error(`Failed to delete local file: ${storagePath}`, error)
    }
  }
}

export async function deleteFile(photo: Photo): Promise<void> {
  const urlsToDelete: (string | null)[] = [photo.url, photo.url_thumb, photo.url_small, photo.url_medium, photo.url_large]
  const deletionPromises = urlsToDelete.filter((url): url is string => !!url).map((url) => deleteFromStorage(url))
