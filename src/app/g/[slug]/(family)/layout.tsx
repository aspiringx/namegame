...
export default async function FamilyGroupLayout({
  ...
  const { members, ...restOfFamilyGroupData } = familyGroupData;

  const currentUserMember = membersWithPhotoUrls.find(m => m.userId === familyGroupData.currentUserMember?.userId);

  const relatedMemberIds = new Set(relationships.flatMap(r => [r.user1Id, r.user2Id]));
  const relatedMembers = membersWithPhotoUrls.filter(m => relatedMemberIds.has(m.userId));
  const notRelatedMembers = membersWithPhotoUrls.filter(m => !relatedMemberIds.has(m.userId));
