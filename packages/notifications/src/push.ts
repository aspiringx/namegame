...
function initializeFirebase() {
  ...
  try {
    ...
    console.error(
      "[Push] First 100 chars of env var:",
      serviceAccountEnv?.substring(0, 100)
    );
  }
}
...
export function configureVapid() {
  ...
  if (!vapidPublicKey || !vapidPrivateKey) {
    throw new Error(
      "VAPID keys not configured. Set NEXT_PUBLIC_WEB_PUSH_PUBLIC_KEY and WEB_PUSH_PRIVATE_KEY"
    );
  }
...
async function sendViaFirebase(
  ...
  console.log(
    "[Firebase] Using stored FCM token:",
  ...
  try {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (result.status === "fulfilled") {
        ...
        failureCount++;
        const error = result.reason as WebPushError & { code?: string };

        // Log detailed error information
        console.error(
          `Push notification failed for subscription ${subscriptions[i].id}:`,
          {
        ...
        // Web Push errors (Safari/Firefox/Edge) - check statusCode
        if (error.statusCode === 410 || error.statusCode === 404) {
          shouldDelete = true;
        }

        // Firebase errors (Chrome/Android) - check error.code or error.message
        if (
        ...
        if (shouldDelete) {
          ...
          console.log(
            `Deleted expired/invalid subscription: ${subscriptions[i].id}`
          );
        } else if (error.statusCode === 401 || error.statusCode === 403) {
          console.error(
            `Authorization error (likely VAPID mismatch) - NOT deleting subscription ${subscriptions[i].id}`
          );
        } else {
          console.error(
            `Unexpected error sending push notification to ${subscriptions[i].id}:`,
            error
          );
        }
      }
