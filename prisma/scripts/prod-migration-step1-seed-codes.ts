...
async function main() {
  ...
  for (const gt of groupTypes) {
    await prisma.groupType.upsert({ where: { id: gt.id }, update: {}, create: gt });
  ...
  for (const role of roleTypes) {
    await prisma.groupUserRole.upsert({ where: { id: role.id }, update: {}, create: role });
  ...
  for (const et of entityTypes) {
    await prisma.entityType.upsert({ where: { id: et.id }, update: {}, create: et });
  ...
  for (const pt of photoTypes) {
    await prisma.photoType.upsert({ where: { id: pt.id }, update: {}, create: pt });
