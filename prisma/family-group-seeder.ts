...
async function main() {
  ...
  if (oldFamilyGroup) {
    console.log('Old family group found. Deleting...')
  ...
  if (!memberRole) {
    throw new Error(
      "'member' role not found. Make sure the base seeder has run.",
    )
  }
  ...
  for (const { path } of relationshipPaths) {
    ...
    for (const step of steps) {
      ...
      const previousUser = await getOrCreateUser(
        previousUserPath,
        memberRole.id,
      )
      ...
      switch (step) {
        ...
        case 'child':
          if (currentUserPath === 'Ego > parent > child') {
            const egoParent1 = await getOrCreateUser(
              'Ego > parent',
              memberRole.id,
            )
            const egoParent2 = await getOrCreateUser(
              'Ego > parent 2',
              memberRole.id,
            )
