...
async function main() {
  ...
  console.log(
    `Ensured group '${testGroup.name}' exists with id: ${testGroup.id}`,
  )
  ...
  for (let i = 0; i < numberOfUsersToCreate; i++) {
    try {
      ...
      console.error(
        `Failed to create user ${i + 1}. It might be a duplicate username or email. Skipping.`,
        error.message,
      )
    }
  }
  ...
  if (!memberRole) {
    throw new Error(
      "Global 'member' role not found. Please run the base seeder first.",
    )
  }
  console.log(`Found 'member' role with id: ${memberRole.id}`)
  ...
  console.log(
    `Successfully added ${createdUsers.length} users to group ${targetGroupId}.`,
  )
  ...
  if (!acquaintanceRelation) {
    throw new Error(
      "'acquaintance' relation type not found. Please run the base seeder first.",
    )
  }
