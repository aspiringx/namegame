...
export default function ParticipantSelector({ 
  ...
  const globalConnectionsWithAvatars: Participant[] = globalConnections.map((conn) => ({
    ...conn,
    avatar: photoMap.get(conn.id) || undefined
  }))

  const participants = mode === 'group' ? groupMembers : globalConnectionsWithAvatars
  ...
  const toggleParticipant = (participantId: string) => {
    setSelectedParticipants(prev => {
      ...
      if (isSelected) {
        ...
        const maxRecipients = mode === 'global' ? MAX_GLOBAL_RECIPIENTS : MAX_GROUP_RECIPIENTS
  ...
  const subtitle = mode === 'group'
    ? `${connectionCount} other group member${connectionCount !== 1 ? 's' : ''}`
          ...
          <div>
            <div className="flex gap-2">
              <button
                ...
                disabled={filteredParticipants.length === 0 || (selectedParticipants.length === filteredParticipants.length) || isAtLimit}
                className="flex-1 px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {wouldExceedLimit ? `Select up to ${maxRecipients}` : 'Select All'}
              </button>
              <button
