...
export function LoginCodeModal({
  ...
  const loginUrl = loginCode
    ? `${window.location.origin}/one-time-login/${loginCode}`
    : ''
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/25 p-4 backdrop-blur-sm">
      <div className="relative my-8 w-full max-w-xl rounded-lg bg-white p-8 text-center shadow-xl bg-gray-800">
        ...
        <h2 className="mb-4 text-2xl font-bold">
          One-Time Login for {user.firstName}
        </h2>
        <p className="mb-4 text-left text-sm text-gray-300">
          Share this QR code or link with {user.firstName} to help them log in.
