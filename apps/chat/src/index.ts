    ...
    pgClient.on('notification', (msg) => {
      try {
        if (msg.channel === 'new_message' && msg.payload) {
          ...
          io.to(`conversation:${messageData.conversationId}`).emit('message', messageData);
          console.log(`[Chat] Broadcasted message to conversation:${messageData.conversationId}`);
        }
      } catch (error) {
    ...
    io.on('connection', async (socket) => {
      ...
      try {
        ...
        socket.on('join-conversation', (conversationId: string) => {
          ...
          console.log(`[Chat] User ${user.id} joined conversation:${conversationId}`);
        });

        // Handle leaving conversation rooms
        socket.on('leave-conversation', (conversationId: string) => {
          socket.leave(`conversation:${conversationId}`);
          console.log(`[Chat] User ${user.id} left conversation:${conversationId}`);
        });
