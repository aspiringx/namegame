...
3. **Convert Scene 1 to Theatre.js** (✓ Complete)
   ...
   - Created keyframes in Studio (0s: opacity=0, 2s: opacity=1)
   - Exported state to `speed-of-love-theatre-state.json`
   - Loaded state in code via `getProject('Speed of Love', { state: theatreState })`
   - Auto-plays on scene load via `theatreProject.ready.then(() => sequence.play())`
   
   **Note:** Currently hardcoding Theatre.js object config in Scene.tsx for learning. Will refactor to load from JSON config file once we understand the flow.

4. **Convert remaining scenes** (Scene 2, 3, 4, etc.)

   - Apply same pattern to each scene
   - Build up complexity incrementally

## Theatre.js State File Structure

The `speed-of-love-theatre-state.json` file stores all animation data (keyframes, timing, values). It's automatically generated by Theatre.js Studio and loaded at runtime.
